...
function normalizeData(input) {
  ...
  const safeCars = cars
    ...
    .map(c => ({
      ...
      km: Number.isFinite(Number(c.km ?? c.odo)) ? Math.max(0, parseInt(c.km ?? c.odo, 10)) : 0,
  ...
  const safeLogs = logs
    ...
    .map(l => ({
      ...
      km: Number.isFinite(Number(l.km ?? l.odo)) ? Math.max(0, parseInt(l.km ?? l.odo, 10)) : 0,
...
function addLog() {
  ...
  const cars = state.data.cars.map(c => String(c.id) === String(f.carId) ? { ...c, km: Math.max(c.km, log.km) } : c);
...
function serviceCard(car, type) {
  ...
  return `
    <div class="service ${overdue ? "overdue" : dueSoon ? "warn" : ""}">
      ...
      ${last ? `
        <div class="small muted" style="margin-top:12px">Último: ${fmtDate(last.date)} · ${last.km.toLocaleString("es-ES")} km</div>
        ${type.interval ? `
          ...
          <div class="small muted" style="margin-top:6px">${since.toLocaleString("es-ES")} / ${type.interval.toLocaleString("es-ES")} km</div>
        ` : ""}
  ...
  <div class="login">
    <div class="card login-card">
      ...
      <p class="small muted" style="margin-top:18px;line-height:1.6">
        Esta versión no expone el token globalmente y usa una política CSP compatible con despliegues estáticos.
      </p>
    </div>
  </div>`;
}
      ...
      <div class="card" style="padding:22px;margin-bottom:20px">
        <div class="row" style="justify-content:space-between;align-items:flex-start">
          ...
          <div style="text-align:right">
            <div class="small muted">Kilometraje</div>
    ...
    <div style="max-width:560px">
      ...
      <div class="card" style="padding:24px">
        <div class="col">
          ...
          <div class="grid-2">
            ${field("Km actuales", `<input class="input" id="car-km" type="number" value="${esc(f.km || "")}" />`)}
      ...
      <div class="card" style="padding:24px">
        <div class="col">
          ...
          <div class="grid-2">
            ...
            ${field("Kilometraje *", `<input class="input" id="log-km" type="number" value="${esc(f.km || "")}" />`)}
          </div>
          <div class="grid-2">
              ...
              <div class="row" style="flex:1;align-items:flex-start">
                ...
                <div>
                  ...
                  <div class="small muted" style="margin-top:6px">
                    📅 ${fmtDate(log.date)} · 🛣️ ${log.km.toLocaleString("es-ES")} km
