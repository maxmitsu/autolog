...
function calculateFuelStats(carId) {
  ...
  for (let i = 1; i < fuelLogs.length; i++) {
    ...
    totalCost += Number(curr.gallons || 0) * Number(curr.pricePerGallon || 0);
  }
...
function getFuelMonthlyStats(carId) {
  ...
  const gallons = monthly.reduce((sum, log) => sum + (Number(log.gallons) || 0), 0);
  const cost = monthly.reduce((sum, log) => {
    const explicit = parseMoney(log.cost);
    if (explicit) return sum + explicit;
    return sum + ((Number(log.gallons) || 0) * (Number(log.pricePerGallon) || 0));
  }, 0);
...
function getLatestFuelLog(carId) {
  const fuelLogs = getFuelLogs(carId);
  return fuelLogs.length ? fuelLogs[fuelLogs.length - 1] : null;
}
...
function addLog() {
  ...
  const log = {
    ...
    gallons: Number.isFinite(Number(f.gallons)) ? Number(f.gallons) : 0,
    pricePerGallon: Number.isFinite(Number(f.pricePerGallon)) ? Number(f.pricePerGallon) : 0,
    fullTank: f.fullTank !== false
  };
  if (log.type === "fuel" && !log.cost && log.gallons && log.pricePerGallon) {
  ...
  return `
    <div class="dashboard-duo">
      <div class="card stat-card">
        ...
        <div class="stat-value">${stats.mpg.toFixed(1)} MPG</div>
        <div class="small muted">Calculado con recargas llenas</div>
      </div>
      <div class="card stat-card">
        <div class="stat-label">💵 Costo por milla</div>
        <div class="stat-value">$${stats.costPerMile.toFixed(2)}</div>
        <div class="small muted">Basado en tus registros de gasolina</div>
      </div>
    </div>
  `;
}
function renderFuelDashboardPanel(car) {
  ...
  return `
    <div class="card fuel-panel">
      ...
      ${latest ? `
        <div class="fuel-latest small muted">
          Última recarga: ${fmtDate(latest.date)} · ${numFmt(latest.gallons || 0, "en-US")} gal · $${numFmt(latest.pricePerGallon || 0, "en-US")}/gal
        </div>
      ` : ``}
    </div>
  `;
        ...
        <div class="col">
          ...
          ${f.type === "fuel" ? `
            <div class="grid-3">
              ...
              ${field("Precio por galón", `<input class="input" id="fuel-price" type="number" step="0.01" value="${esc(f.pricePerGallon || "")}" />`)}
              ${field("Tanque lleno", `<select class="select" id="fuel-fullTank">
                <option value="yes" ${f.fullTank !== false ? "selected" : ""}>Sí</option>
                <option value="no" ${f.fullTank === false ? "selected" : ""}>No</option>
              </select>`)}
            </div>
          ` : ``}
...
function bindEvents() {
  ...
  $("#save-log-btn")?.addEventListener("click", () => {
    state.form = {
      ...
      pricePerGallon: $("#fuel-price")?.value || "",
      fullTank: ($("#fuel-fullTank")?.value || "yes") === "yes",
    };
    addLog();
  });
  $("#log-type")?.addEventListener("change", e => { state.form = { ...state.form, type: e.target.value }; render(); });
